...
namespace AuthMicroservice.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public AuthenticationService(AuthDbContext context, IPasswordHasher passwordHasher, ITokenProvider tokenProvider)
        {
            _context = context;
            _passwordHasher = passwordHasher;
            _tokenProvider = tokenProvider;
        }
        public async Task Registration(string email, string password,string nickname)
        {
            ...
            UserEntity newUser = UserEntity.Create(Guid.NewGuid(), email, hashedPassword,nickname);
            if (users == 0)
